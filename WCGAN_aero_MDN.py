...
#import data
DATASET_PATH = './datasets/aero_MDN_2200/'
DATASET_NAME = 'YawBrMyn_ST_DEL'
CHANNEL_NAME = "YawBrMyn_[kN-m] ST_DEL"

# For saving plots
PLOT_PATH = './plots'
PLT_DATASET_NAME = 'aero_MDN_{}'.format(DATASET_NAME)

# path for saving parameters of model
PARAM_PATH = './param_best'
FILE_NAME = 'aero_MDN_wcgan_samplesize_2200'
...
savepath = os.path.join(PLOT_PATH,PLT_DATASET_NAME,FILE_NAME)
f = open('{}/output.txt'.format(savepath), 'w')
sys.stdout = f
print(config)
print(nn_spec)


# with open('{}/nn_spec.txt'.format(savepath), 'w') as f:
#     f.write(json.dumps(nn_spec))
