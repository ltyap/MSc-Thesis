        ...
        for epoch in range(self.config["epochs"]):
            ...
            for iter, (x_batch, data_batch) in enumerate(train_loader):
                ...
                for _ in range(self.n_critic):
                    ...
                    gen_logits = self.critic(torch.cat((x_batch,gen_output.detach()), dim = 1))
                    #gradient penalty
                    gp = self.compute_gradient_penalty(x_batch, gen_output.detach(), data_batch)

                    disc_loss = self.disc_loss(gen_logits, data_logits, gp)
                    mean_iteration_critic_loss += disc_loss.item() /self.n_critic
                    disc_loss.backward(retain_graph=True)
                    disc_opt.step()

                self.discriminator_loss.append(mean_iteration_critic_loss)
                ...
                gen_logits_val = self.critic(torch.cat((x_batch_val,gen_output_val.detach()), dim = 1))
                data_logits_val = self.critic(torch.cat((x_batch_val,data_batch_val), dim = 1))
                gp_val = self.compute_gradient_penalty(x_batch_val, gen_output_val.detach(), data_batch_val)
                disc_loss_val = self.disc_loss(gen_logits_val, data_logits_val, gp_val)

                self.discriminator_loss_val.append(disc_loss_val.item())

                # Calculate Wasserstein-2 distance between
                # generated samples and real samples from validation set
                gen_output_val = gen_output_val.cpu().detach().numpy()
                M_wasserstein = ot.wasserstein_1d(gen_output_val, val_tab.ys.detach().numpy(), p = 2)**0.5
                real_samples_std = np.std(val_tab.ys.detach().numpy(), axis=0)
                M_wasserstein_normalised = M_wasserstein/real_samples_std
                self.wasserstein_dist.append(M_wasserstein_normalised.mean())
