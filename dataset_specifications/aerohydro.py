...
class AeroHydroSet(Dataset):
    def __init__(self) -> None:
     ...
     'YawBrMyn_[kN-m] mean', 'YawBrMyn_[kN-m] max',
       'YawBrMyn_[kN-m] stddev', 'YawBrMyn_[kN-m] ST_DEL']
        ...
        self.dataset_save_path = os.path.join("datasets",self.name,self.tmp[self.channel_name])
    def load_data(self):
        ...
        columns = self.inputs[:]
        columns.append(self.channel_name)
    ...
    def plot_test_data(self, test_data_scaled):
        channel = self.tmp[self.channel_name]
        plot_path = './plots/{}_{}'.format(self.name, channel)
        ...
        for input in self.inputs:
            plt.figure()
            plt.scatter(test_data_scaled[input],test_data_scaled[self.channel_name])
            plt.xlabel(input)
            plt.ylabel(channel)
            plt.savefig(os.path.join(plot_path, '{}_{}'.format(plot_name,input)))
            plt.close()
        test_scaled_LD = dataset.LabelledData(x= test_data_scaled.to_numpy()[:,:self.x_dim],
                                          y = test_data_scaled.to_numpy()[:,self.x_dim:])
        ...
        for _, (start, end) in enumerate(zip(start_idx,end_idx)):
            ...
            plt.title('x={}, idx = {}-{}'.format(test_LD.x[start], start, end-1), fontsize=9)
